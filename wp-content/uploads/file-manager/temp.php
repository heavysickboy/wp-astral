 ...
 *
 * This template yourtheme/woocommerce/ti-wishlist.php.
 *
 * @version             1.24.5
		...
		<table class="tinvwl-table-manage-list">
			...
			<tr>
				...
				<?php if (isset($wishlist_table_row['colm_price']) && $wishlist_table_row['colm_price']) { ?>
					<th class="product-price"><?php esc_html_e('Price', 'ti-woocommerce-wishlist'); ?></th>
				<?php } ?>
				...
				<?php } ?>
				<!--<th class="product-desription">Product Description</th>-->
				<!--<th class="product-desription">Enquiry Now</th>-->
			</tr>
			</thead>
			<tbody>
           ...
           // echo '<pre>';
			...
			foreach ($products as $wl_product) {
				...
				if ($wl_product['quantity'] > 0 && apply_filters('tinvwl_wishlist_item_visible', true, $wl_product, $product)) {
					...
					<tr class="<?php echo esc_attr(apply_filters('tinvwl_wishlist_item_class', 'wishlist_item', $wl_product, $product)); ?>">
						...
						<td id="proID<?php echo $proID;?>" class="product-name">
							<?php
							if (!$product->is_visible()) {
			...
			<?php do_action('tinvwl_wishlist_contents_after'); ?>
			    <tr><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;<strong><span style="font-size: 13px;">* Dear customer help us to know your requirment.</span> <i class="bi bi-arrow-right"></i></strong></td>
			      <td class="product-enq"><a href="#" data-fancybox="" data-src="#applyJob1" class="ctaBtn blackBtn py-1">Enquire Now <i class="bi bi-pencil-square"></i></a></td>
			    </tr>
		 	</tbody>
			<tfoot>
			<tr>
...
<div class="contactForm">
  ...
  foreach($products as $f_product){ 
     $dataset = $products[0]['data'];
     $product_id = $f_product['product_id'];
     $e_product = wc_get_product( $product_id);
   ?>
    <input type="hidden" name="wishproducts" value="<?php echo $e_product->get_name() ; ?>" />
  <?php } ?>
   <?php echo do_shortcode('[contact-form-7 id="2059" title="Wishlist Product Inquiry"]');?>
  </div>
</div>      
...
jQuery(".tinvwl-table-manage-list tr.wishlist_item").each(function(){
	var val = jQuery(this).children('td.product-name').children('a').html(); var value = ( val + ' || '); gethtml += value; 
